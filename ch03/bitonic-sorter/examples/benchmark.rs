...
use bitonic_sorter::{
    fourth::sort as par_sort,
    third::sort as seq_sort,
    utils::{is_sorted_ascending, new_u32_vec},
    SortOrder,
};

fn main() {
    if let Some(n) = env::args().nth(1) {
        let bits = u32::from_str_radix(&n, 10).expect("error parsing argument");
        run_sorts(bits);
    } else {
        eprintln!(
    ...
    F: Fn(&mut [u32], &SortOrder) -> Result<(), String>,
{
    ...
    let nano_secs = dur.as_nanos() as f64 + dur.as_secs_f64() * 1e9_f64;
