    ...
    F: Sync + Fn(&T, &T) -> Ordering,
{
    if x.len().is_power_of_two() {
        ...
        Err(format!(
            "The length of x is not power of two, (x.len(): {})",
            x.len()
        ))
    }
}
    ...
    F: Fn(&T, &T) -> Ordering,
{
    ...
    for i in 0..mid_point {
        if comparator(&x[i], &x[mid_point + i]) == swao_condition {
...
mod tests {
    ...
    use crate::{
        utils::{is_sorted_ascending, is_sorted_descending, new_u32_vec},
        SortOrder::*,
    };
